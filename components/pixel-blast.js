...
const disposeThree = (t, container) => {
  ...
  else if (canvas?.parentElement) canvas.parentElement.removeChild(canvas);
};
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    const {
      ...
      liquid: useLiquid,
      liquidStrength: lStrength,
      liquidRadius: lRadius,
      pixelSizeJitter: jitter,
      enableRipples: ripples,
      rippleIntensityScale: rIntensity,
      rippleThickness: rThickness,
      rippleSpeed: rSpeed,
      liquidWobbleSpeed: lWobble,
      autoPauseOffscreen: pauseOffscreen,
    ...
    const uniforms = {
      ...
      uShapeType: { value: SHAPE_MAP[v] ?? 0 },
      uPixelSize: { value: px * renderer.getPixelRatio() },
      uScale: { value: scale },
      uDensity: { value: density },
      uPixelJitter: { value: jitter },
      uEnableRipples: { value: ripples ? 1 : 0 },
      uRippleSpeed: { value: rSpeed },
      uRippleThickness: { value: rThickness },
      uRippleIntensity: { value: rIntensity },
      uEdgeFade: { value: fade },
    };
    ...
    // Pass display-ready sRGB channel values so the pattern stays vivid.
    ...
    const setSize = () => {
      ...
      uniforms.uPixelSize.value = px * renderer.getPixelRatio();
    };

    if (useLiquid) {
      touch = createTouchTexture();
      touch.radiusScale = lRadius;
    ...
    if (noise > 0) {
      ...
      const noiseEffect = new Effect(
        ...
        `uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.0,1080.0))+floor(uTime*60.0)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.0);} `,
        {
          uniforms: new Map([
            ["uTime", new THREE.Uniform(0)],
            ["uAmount", new THREE.Uniform(noise)],
          ]),
        }
      );
    ...
    let raf = 0;
    const animate = () => {
      if (pauseOffscreen && !visibilityRef.current.visible) {
  ...
  useEffect(() => {
    ...
    if (typeof color === "string" && color.startsWith("#") && color.length >= 7) {
      t.uniforms.uColor.value.setRGB(
    ...
    t.renderer.setClearColor(0x000000, transparent ? 0 : 1);
    if (t.liquidEffect) {
